...
png(args[3])
plot(pcaz$eigenvect[,1][missingRate < 0.5], pcaz$eigenvect[,2][missingRate < 0.5], xlab=paste("PC1=", pcaz$varprop[1]*100, "%", sep=""), ylab=paste("PC2=", pcaz$varprop[2]*100, "%", sep=""),col=as.character(metaz$Color[order(as.character(metaz$Individual))])[missingRate < 0.5], pch=20)
dev.off()
